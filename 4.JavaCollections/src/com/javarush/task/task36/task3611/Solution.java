...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        Set friendsAndPotentialFriends = new HashSet();
        if (deep == 0)
            return friendsAndPotentialFriends;

        for (int j = 0; j < humansRelationships.length; j++) {
            if (j < index && humansRelationships[index][j]) {
                friendsAndPotentialFriends.add(j);
                friendsAndPotentialFriends.addAll(getAllFriendsAndPotentialFriends(j, deep - 1));
            } else if (j > index && humansRelationships[j][index]) {
                friendsAndPotentialFriends.add(j);
                friendsAndPotentialFriends.addAll(getAllFriendsAndPotentialFriends(j, deep - 1));
            }
        }
        friendsAndPotentialFriends.remove(index);
        return friendsAndPotentialFriends;//напишите тут ваш код
    }
